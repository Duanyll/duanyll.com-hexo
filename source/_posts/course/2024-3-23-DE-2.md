...
$$
c\frac{2h\beta-\gamma y_{n-1}+4\gamma y_n}{2h+3\gamma}+a_ny_n+b_ny_{n-1}=h^2f(x_n)
$$

$$
(b_n-\frac{c_N\gamma}{2h+3\gamma})y_{n-1}+(a_n+\frac{4c_n}{2h+3\gamma})y_n=h^2f_n-\frac{2c_nh\beta}{2h+3\gamma}
$$
...
\begin{pmatrix}
    ...
    & & & & b_n-\frac{c_N\gamma}{2h+3\gamma} & a_n+\frac{4c_n}{2h+3\gamma}
...
\begin{cases}
    y_0-y_{n+1}=0\\
    4y_1-y_2-y_{n-1}+4y_n=0
\end{cases}
$$

$y_0$ and $y_{n+1}$ have to be append into the FDE system to solve
...
$$
\begin{aligned}
    y(x_{i+1})-2y(x_i)+y(x_{i-1})=&y(x_i)+hy'(x_i)+\frac{h^2}{2}y''(x_i)+\frac{h^3}{6}y'''(x_i)+\frac{h^4}{24}y''''(x_i)+\frac{h^5}{120}y^{(5)}(x_i)+O(h^6)\\
    &-2y(x_i)+\\
    &y(x_i)-hy'(x_i)+\frac{h^2}{2}y''(x_i)-\frac{h^3}{6}y'''(x_i)+\frac{h^4}{24}y''''(x_i)-\frac{h^5}{120}y^{(5)}(x_i)+O(h^6)\\
