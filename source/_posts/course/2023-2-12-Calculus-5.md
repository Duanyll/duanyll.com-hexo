...
1. $\left\{\sum_{n=1}^\infty a_n\right\}$ 有界
2. $\{b_n\}$ 单调收敛到 $0$
...
1. $\sum_{n=1}^\infty a_n$ 绝对收敛 $\implies \sum_{n=1}^\infty a^+_n, \sum_{n=1}^\infty a^-_n$ 收敛
...
**定理** 逐项求导定理: $\{f_n(x)\}$ 在区间 $\mathcal{I}$ 上一致收敛到 $f(x)$, $f_n\in C^1(\mathcal{I})\implies f\in C^1(\mathcal{I})$ 且

$$
f'(x)=\left(\lim_{n\to\infty}f_n(x)\right)'=\lim_{n\to\infty}f'_n(x)
$$
